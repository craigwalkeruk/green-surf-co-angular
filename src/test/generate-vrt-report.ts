...
<body>
  ...
  <script>
    ...
    function renderSummary() {
      ...
      const withDiff = comparisons.filter(c => c.hasDiff).length;
      document.getElementById('summary').innerHTML = \`
        <span>Total: <strong>\${total}</strong></span>
        <span class="fail">With differences: <strong>\${withDiff}</strong></span>
        <span class="pass">Matching: <strong>\${total - withDiff}</strong></span>
      \`;
    }
      ...
      card.innerHTML = \`
        <div class="test-header" onclick="toggleCard(\${index})">
          ...
          <div style="display: flex; align-items: center; gap: 1rem;">
            <span class="test-status \${comparison.hasDiff ? 'diff' : 'match'}">
              \${comparison.hasDiff ? 'DIFF' : 'MATCH'}
            </span>
...
export default class VrtReporter implements Reporter {
  onTestRunEnd(): void {
    // Generate report after tests complete (async, fire-and-forget for watch mode)
...
if (isMainModule) {
  generateReport(false).then((reportPath) => {
    if (!reportPath) {
      console.log('Run your visual regression tests first.');
      process.exit(0);
    }
  }).catch(console.error);
}
